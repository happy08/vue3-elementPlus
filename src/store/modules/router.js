...
const router = {
  ...
  mutations: {
    ...
    SET_ROUTERS: (state, data) => {
      ...
      state.addRouters = markRaw([...addRouters, ...localRouters, ...errorRouters]); //(错误路由一定要放在最后添加，否则页面会被错误路由劫持)
      //将当前已存在的路由与动态添加的路由合并保存
      state.routers = markRaw([...commontRouters, ...state.addRouters]);
    },
    //清除动态添加的路由
    ...
    GenerateRoutes({ state, commit }, data) {
      return new Promise(resolve => {
        ...
        getMenuList(send).then(res => {
          ...
          let addRoutes = toRaw(state.addRouters);
          console.log("过滤后的动态路由，注入", addRoutes);

          //注入到路由
          for (let i = 0; i < addRoutes.length; i++) {
            Router.addRoute(addRoutes[i])
          }

          //Router.replace(Router.currentRoute.value.fullPath)
          //Router.addRoute(addRoutes); // 动态添加可访问路由表
